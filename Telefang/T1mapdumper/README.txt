...
This is version 2.0 of the program, which now uses PNG images as the output format instead of PGM and PPM formats. This change makes it much easier to be able to view the images in the first place, since PNG is much more widespread than PGM or PPM. Additionally, these images are now losslessly compressed, resulting in a smaller filesize, and alpha transparency is now used for invalid acres, as opposed to making the pixels darker.

To accomplish the ability to output PNG images, the PyPNG library is used. It is included with this program, so you don't need to download anything extra, but you can always download it here if you want to use this library in any of your own Python programs: http://pypi.python.org/pypi/pypng/0.0.13
...
Once Python 2.7 is installed, you can start the Telefang Map Dumper by double-clicking on mapdump.py. If done correctly, a terminal window should pop up.
...
The program will write to "outputmap.png". Due to them being in the PNG format, the maps will be losslessly compressed. The maps will all be 1280x1024. If the grayscale option is chosen, the filesize will be smaller.
